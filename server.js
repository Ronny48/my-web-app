...
//make our markdown function available in our templates
    res.locals.filterUserHTML = function(content) {
        return sanitizeHTML( marked.parse(content) , {
            allowedTags: ['p', 'br','ul', 'li', 'ol', 'strong', 'bold', 'i', 'em', 'u', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'], 
            allowedAttributes: {}
        })
    }
...
app.post('/login', (req, res) => {
    ...
    const ourTokenValue = jwt.sign({exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24, 
        skycolor: 'blue', userid: userInQuestion.id, username: userInQuestion.username}, process.env.JWTSECRET)
...
app.get('/post/:id', (req, res) => {
    const ourStatement = db.prepare('SELECT posts.*, users.username FROM posts INNER JOIN users ON posts.authorId = users.id WHERE posts.id = ?')
...
app.post('/create-post', mustBeLoggedIn, (req, res) => {
   ...
   const result = ourStatement.run(req.body.title, 
    req.body.body, req.user.userid, new Date().toISOString())
...
app.post('/register', (req, res) => {
    ...
    if(req.body.username && req.body.username.length > 10) errors.push('Username cannot be more than 10 characters')
